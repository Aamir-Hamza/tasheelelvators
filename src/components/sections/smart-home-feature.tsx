      ...
      <div className="relative mx-auto grid max-w-7xl lg:grid-cols-2">
        ...
        >
          ...
          <h2 className="mt-4 font-display text-3xl font-bold tracking-tight text-white md:text-4xl lg:text-[2.75rem] lg:leading-[1.1]">
            Smart locks that secure the whole home.
          </h2>
          <p className="mt-4 max-w-md text-base leading-relaxed text-stone-300">
            Fingerprint, NFC, and remote access — linked to CCTV, alarms, and
            automation so every entry is monitored and every home feels smarter.
          </p>

          <ul className="mt-8 grid gap-4 sm:grid-cols-3">
            {[
              { icon: Fingerprint, label: "0.3s fingerprint unlock" },
              { icon: Wifi, label: "App & NFC access" },
              { icon: ShieldCheck, label: "Bank-level cylinder" },
            ].map(({ icon: Icon, label }) => (
              <li
                key={label}
                className="flex items-start gap-3 border-l border-amber-500/25 pl-3"
              >
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            ...
            >
              Explore Smart Systems <ArrowRight className="h-4 w-4" />
            </Link>
            <Link
